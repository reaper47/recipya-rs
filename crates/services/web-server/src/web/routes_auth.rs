...
pub fn routes(mm: ModelManager) -> Router {
    Router::new()
        ...
        .route("/forgot-password/reset", post(handlers_auth::forgot_password_reset_post))
        .route("/login", get(handlers_auth::login).post(handlers_auth::login_post))
        .route("/logout", post(handlers_auth::logout_post))
        .route("/register", get(handlers_auth::register).post(handlers_auth::register_post))
        .with_state(mm)
}
