...
mod tests {
    ...
    async fn prepare_server_with_recipes(
        config: Config,
    ) -> Result<(TestServer, Vec<RecipeImages>)> {
        ...
        let images = insert_recipes(&state.mm, user_id).await?;
        Ok((server, images))
    }
    ...
    async fn test_page_below_1_returns_recipes_from_first_page() -> Result<()> {
        ...
        let (server, _) = prepare_server_with_recipes(config).await?;
    ...
    async fn test_results_query1() -> Result<()> {
        let (_test_db, config) = TestDb::new(None).await?;
        let (server, images) = prepare_server_with_recipes(config).await?;
