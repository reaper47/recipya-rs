    ...
    fn assert_content(res: &TestResponse) {
        assert_html(
            ...
            vec![
                ...
                r##"<dialog class="modal" id="import-recipes-dialog"><div id="import-recipes-dialog-container" class="modal-box max-w-none w-96 max-h-[92vh] p-4 flex flex-col"><form method="dialog"><button class="btn btn-sm btn-circle btn-ghost absolute right-2 top-2">✕</button></form><h3 class="font-bold text-lg">Import Recipes</h3><div class="tabs tabs-lift pt-4"><label class="tab"><input type="radio" name="import-recipe-tab" checked _="on click set #import-recipes-dialog-container.style.width to ''">Software</label><div class="tab-content bg-base-100 border-base-300 p-3"><form class="space-y-4" enctype="multipart/form-data" hx-post="/recipes/add/import/app" hx-indicator="#fullscreen-loader" hx-swap="none" hx-on:htmx:before-request="if(!this.checkValidity()) return false; document.querySelector('#import-recipes-dialog').close()"><div><fieldset class="fieldset"><legend class="fieldset-legend">Choose an application</legend><select class="select" id="app-select" name="app"><option disabled selected>Pick an application</option><option value="accuchef">AccuChef</option><option value="bigoven">BigOven</option><option value="cheftap">ChefTap</option><option value="cooklang">Cooklang</option><option value="cookmate">CookMate</option><option value="crouton">Crouton</option><option value="kalorio">Kalorio</option><option value="mastercook">MasterCook</option><option value="mealmaster">MealMaster</option><option value="paprika">Paprika</option><option value="recipemd">RecipeMD</option><option value="recipesage">RecipeSage</option><option value="rezkonv">Rezkonv</option><option value="saffron">Saffron</option></select></fieldset><fieldset class="fieldset"><legend class="fieldset-legend">Select a file</legend><input id="import-dialog-file" type="file" name="file" required class="file-input w-full""##,
            ],
        );
    }
}
