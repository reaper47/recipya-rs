...
mod tests {
  ...
  "recipeYield": "1 loaf",
    ...
    fn other_valid_preview_form() -> PreviewForm {
        PreviewForm {
            json_input: json!({
              "@context": "https://schema.org",
              "@type": "Recipe",
    ...
    mod tests_recipe_add_import_preview {
        ...
        #[tokio::test]
        async fn test_post_payload_with_properties_ok() -> Result<()> {
            let (_test_db, config) = TestDb::new(None).await?;
            let server = build_server_logged_in(config).await?;

            let res = server
                .post(BASE_URI)
                .form(&other_valid_preview_form())
                .await;
            ...
            assert_html(
                ...
                vec![
                    ...
                    r#"<ul class="list grid gap-1"><li class="list-row py-1 no-after select-none grid grid-cols-1 hover:bg-base-300"><label class="flex items-center w-full"><input type="checkbox" class="checkbox"><span class="pl-2">1 slice whole wheat bread</span></label></li></ul>"#,
                ],
            );
            Ok(())
        }
