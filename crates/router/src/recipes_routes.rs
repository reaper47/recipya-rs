...
mod tests {
    ...
    mod tests_favourite_recipe {
        ...
        async fn test_recipe_does_not_exist_ok() -> Result<()> {
            ...
            let res = server.post(&base_uri(20)).await;
        ...
        async fn test_mark_as_favourite_ok() -> Result<()> {
            ...
            assert_html(
                res,
                vec![
                    r##"<button id="favourite-1" class="btn btn-square absolute top-2 right-2 hover:text-secondary" hx-post="/recipes/1/favourite" hx-target="#favourite-1" hx-swap="outerHTML" hx-push-url="false" aria-label="Add to favorites" aria-pressed="true" _="on mousedown halt the event"><svg xmlns="http://www.w3.org/2000/svg" fill="currentColor""##,
                ],
            );
            Ok(())
        }
        ...
        async fn test_mark_as_favourite_already_favourite_ok() -> Result<()> {
            ...
            assert_html(
                res,
                vec![
                    r##"<button id="favourite-1" class="btn btn-square absolute top-2 right-2 hover:text-secondary" hx-post="/recipes/1/favourite" hx-target="#favourite-1" hx-swap="outerHTML" hx-push-url="false" aria-label="Add to favorites" aria-pressed="false" _="on mousedown halt the event"><svg xmlns="http://www.w3.org/2000/svg" fill="none""##,
                ],
            );
            Ok(())
        }
        ...
        async fn test_mark_twice_in_a_row_ok() -> Result<()> {
            ...
            let _ = server.post(&base_uri(1)).await;
            let res = server.post(&base_uri(1)).await;
