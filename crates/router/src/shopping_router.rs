...
use crate::{
    handlers::shopping::{
        ...
        shopping_list_item_toggle_handler, shopping_list_label_put_handler,
    ...
    mod tests_list_items {
        ...
        async fn test_item_valid_ok() -> Result<()> {
            ...
            assert_html(
                ...
                &[
                    &format!(
                        r#"<li class="list-row grid grid-cols-[1fr_auto]"><div class="grid gap-1 min-w-0"><label class="label text-base-content"><input type="checkbox" class="checkbox peer" hx-post="/shopping/lists/{}/items/1/toggle"><span class="[input:checked~&amp;]:line-through [input:checked~&amp;]:opacity-50 transition-all">Spaghetti (500g)</span></label></div><div class="flex gap-1">"#,
                        c.list_id
                    ),
                    &format!(
                    ...
                    r#"<button class="btn join-item btn-square btn-sm cursor-grab h-full [li:has(input:checked)_&amp;]:hidden transition-all"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6"><path stroke-linecap="round" stroke-linejoin="round" d="M3 7.5 7.5 3m0 0L12 7.5M7.5 3v13.5m13.5 0L16.5 21m0 0L12 16.5m4.5 4.5V7.5"></svg></button>"#,
                ],
            );
            Ok(())
        }
        ...
        async fn test_put_item_ok() -> Result<()> {
            ...
            assert_html(
                ...
                &[
                    &format!(
                        r#"<li class="list-row grid grid-cols-[1fr_auto]"><div class="grid gap-1 min-w-0"><label class="label text-base-content"><input type="checkbox" class="checkbox peer" hx-post="/shopping/lists/{list_id}/items/1/toggle"><span class="[input:checked~&amp;]:line-through [input:checked~&amp;]:opacity-50 transition-all">Apples (50)</span></label></div><div class="flex gap-1">"#
                    ),
                    &format!(
                        r#"<button class="btn join-item btn-square btn-sm [li:has(input:checked)_&amp;]:hidden transition-all" hx-target="closest li" hx-swap="outerHTML" hx-get="/shopping/lists/{list_id}/items/1/edit"><svg xmlns="http://www.w3.org/2000/svg" class="size-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg></button>"#
                    ),
