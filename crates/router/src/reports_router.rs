    ...
    mod tests_reports {
        ...
        async fn test_reports_exists_ok() -> Result<()> {
            ...
            assert_html(
                &res,
                vec![
                    r##"<div id="report-index" class="flex flex-col md:flex-row h-full"><aside class="relative max-h-full" hx-get="/reports/list" hx-trigger="refreshReports from:body" hx-target="#report-list-container" hx-include="#selected-report-id"><input id="selected-report-id" type="hidden" name="selected" value="0"><div id="report-list-container"><ul id="report-menu" class="menu block bg-base-100 w-full overflow-y-auto max-h-[89vh] h-full"><li class="bg-base-300" hx-get="/reports/2" hx-target="#report-view-pane" hx-push-url="true" hx-trigger="mousedown" hx-on:mousedown="document.querySelectorAll('#report-menu li').forEach((el) =&gt; el.classList.remove('bg-base-300')); this.classList.add('bg-base-300'); document.getElementById('selected-report-id').value = '2';"><div class="grid"><div class="grid place-self-start"><p class="font-bold">"##,
                    ...
                    r#"<footer id="pagination-reports" class="footer footer-center bg-base-200 p-2 gap-2 pb-16 md:pb-2 mt-auto shrink-0 absolute bottom-0" style="grid-auto-flow: row;" onload="updateAddCookbookUrl(1)"><div class="join gap-0"><button class="join-item btn btn-disabled btn-xs md:btn-sm w-8 md:w-12" title="Previous page" aria-label="Previous page">‹</button><button class="join-item btn btn-active btn-xs md:btn-sm w-8 md:w-12" aria-current="page" aria-label="Page 1, current page">1</button><button class="join-item btn btn-disabled btn-xs md:btn-sm w-8 md:w-12" title="Next page" aria-label="Next page">›</button></div><div class="text-center"><p class="text-xs md:text-sm">Showing <span class="font-semibold text-base-content">1</span>-<span class="font-semibold text-base-content">2</span> of <span id="search-count" class="font-medium">2</span> results</p></div></footer>"#,
                ],
            );
            Ok(())
        }
    ...
    mod tests_report {
        ...
        async fn test_reports_exists_ok() -> Result<()> {
            ...
            assert_html(
                &res,
                vec![
                    r##"<div id="report-index" class="flex flex-col md:flex-row h-full"><aside class="relative max-h-full" hx-get="/reports/list" hx-trigger="refreshReports from:body" hx-target="#report-list-container" hx-include="#selected-report-id"><input id="selected-report-id" type="hidden" name="selected" value="1"><div id="report-list-container"><ul id="report-menu" class="menu block bg-base-100 w-full overflow-y-auto max-h-[89vh] h-full"><li hx-get="/reports/2" hx-target="#report-view-pane" hx-push-url="true" hx-trigger="mousedown" hx-on:mousedown="document.querySelectorAll('#report-menu li').forEach((el) =&gt; el.classList.remove('bg-base-300')); this.classList.add('bg-base-300'); document.getElementById('selected-report-id').value = '2';"><div class="grid"><div class="grid place-self-start"><p class="font-bold">"##,
                    r#"<footer id="pagination-reports" class="footer footer-center bg-base-200 p-2 gap-2 pb-16 md:pb-2 mt-auto shrink-0 absolute bottom-0" style="grid-auto-flow: row;" onload="updateAddCookbookUrl(1)"><div class="join gap-0"><button class="join-item btn btn-disabled btn-xs md:btn-sm w-8 md:w-12" title="Previous page" aria-label="Previous page">‹</button><button class="join-item btn btn-active btn-xs md:btn-sm w-8 md:w-12" aria-current="page" aria-label="Page 1, current page">1</button><button class="join-item btn btn-disabled btn-xs md:btn-sm w-8 md:w-12" title="Next page" aria-label="Next page">›</button></div><div class="text-center"><p class="text-xs md:text-sm">Showing <span class="font-semibold text-base-content">1</span>-<span class="font-semibold text-base-content">2</span> of <span id="search-count" class="font-medium">2</span> results</p></div></footer>"#,
