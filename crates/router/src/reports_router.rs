        ...
        async fn test_reports_exists_ok() -> Result<()> {
            ...
            assert_html(
                ...
                vec![
                    ...
                    r#"<footer id="pagination-reports" class="footer footer-center bg-base-200 p-2 gap-2 md:pb-2 mt-auto shrink-0 absolute bottom-0" style="grid-auto-flow: row;" onload="updateAddCookbookUrl(1)"><div class="join gap-0"><button class="join-item btn btn-disabled w-12" title="Previous page" aria-label="Previous page">‹</button><button class="join-item btn btn-active w-12" aria-current="page" aria-label="Page 1, current page">1</button><button class="join-item btn btn-disabled w-12" title="Next page" aria-label="Next page">›</button></div><div class="text-center"><p class="text-sm text-base-content/70">Showing <span class="font-semibold text-base-content">1</span>-<span class="font-semibold text-base-content">2</span> of <span id="search-count" class="font-medium">2</span> results</p></div></footer>"#,
                ],
            );
            Ok(())
        }
