...
use test_harness::{assert_html, assert_not_in_html};
...
async fn test_get_user_has_recipes_ok() -> Result<()> {
    ...
    let mut recipe_ids = Vec::with_capacity(3);
    for i in 0..3 {
    ...
    let id1 = recipe_ids[0];
    let id2 = recipe_ids[1];
    let id3 = recipe_ids[2];
