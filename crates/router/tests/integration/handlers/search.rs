...
use test_harness::{assert_html, assert_must_be_logged_in, build_server_logged_in};
...
async fn insert_recipes(mm: &ModelManager, user_id: Uuid) -> Result<(Vec<i64>, Vec<RecipeImages>)> {
...
async fn test_results_query1() -> Result<()> {
    let ((server, _), (recipe_ids, images)) = prepare_server_with_recipes(default_config()).await?;
