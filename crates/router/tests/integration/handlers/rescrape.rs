...
mod tests_get {
    ...
    use test_harness::assert_sse_messages_any_order;
    ...
    async fn test_scraped_recipe_same_as_old_ok() -> Result<()> {
        ...
        assert_sse_messages_any_order(
                    &mut ws_server,
                    &[
                        r#"{"headers": {"HX-Trigger": "refreshReports"}}"#,
                        r#"{"notification":{"type":"toast","message":"Recipe has not changed.","status":"alert-warning","title":"Attention"}}"#,
                    ],
                )
                .await;
        let user_id = User::all(&state.mm).await?[0].id;
...
mod tests_put {
    ...
    async fn test_scraped_recipe_diff_title_ok() -> Result<()> {
        ...
        let recipe_id = insert_zweigles_recipe(&server, &mut ws_server, &state).await?;
        let user_id = User::all(&state.mm).await?[0].id;
        let form: Vec<(&str, &str)> = vec![("title-source", "new"), ("title-new", "Bluerry Pie")];
    ...
    async fn test_scraped_recipe_diff_category_ok() -> Result<()> {
        ...
        let recipe_id = insert_zweigles_recipe(&server, &mut ws_server, &state).await?;
        let user_id = User::all(&state.mm).await?[0].id;
        let form: Vec<(&str, &str)> = vec![("category-source", "new"), ("category-new", "lunch")];
    ...
    async fn test_scraped_recipe_diff_cuisine_ok() -> Result<()> {
        ...
        let recipe_id = insert_zweigles_recipe(&server, &mut ws_server, &state).await?;
        let user_id = User::all(&state.mm).await?[0].id;
        let form: Vec<(&str, &str)> = vec![("cuisine-source", "new"), ("cuisine-new", "tunisian")];
    ...
    async fn test_scraped_recipe_diff_instructions_grouped_ok() -> Result<()> {
        ...
        let mut form: Vec<(String, String)> = vec![("instructions-source".into(), "new".into())];
        for (section_title, items) in &sections {
            for item in items {
                form.push((
...
async fn insert_zweigles_recipe(
    ...
    assert_sse_message(ws_server, &format!(r#"{{"notification":{{"type":"toast","action":"View /recipes/{last_recipe_id}","message":"Recipe has been added to your collection.","status":"alert-info","title":"Success"}}}}"#)).await;
    Ok(last_recipe_id)
}
