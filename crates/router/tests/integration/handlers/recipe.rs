...
use test_harness::{
    assert_html, assert_must_be_logged_in, assert_not_in_html, assert_sse_message,
...
mod tests_get {
    ...
    async fn test_recipe_exists_ok() -> Result<()> {
        ...
        assert_complete_recipe(&res, &recipe, recipe_id, &images);
        Ok(())
    }
    ...
    async fn test_recipe_exists_flat_ingredients_instructions_ok() -> Result<()> {
        ...
        recipe.ingredients = SectionComponents::Flat(vec![Item::new("ing1"), Item::new("ing2")]);
        recipe.instructions = SectionComponents::Flat(vec![Item::new("ins1"), Item::new("ins2")]);
        let recipe_id = Recipe::create(&state.mm, user_id, &recipe, &settings).await?;
    ...
    async fn test_source_is_not_link() -> Result<()> {
        ...
        assert_not_in_html(
            &res,
            &[&format!(
                r##"<li _="on click document.activeElement.blur()"><button hx-get="/recipes/{recipe_id}/rescrape" hx-target="#content" hx-swap="none" hx-indicator="#fullscreen-loader"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6"><path stroke-linecap="round" stroke-linejoin="round" d="M12 21a9.004 9.004 0 0 0 8.716-6.747M12 21a9.004 9.004 0 0 1-8.716-6.747M12 21c2.485 0 4.5-4.03 4.5-9S14.485 3 12 3m0 18c-2.485 0-4.5-4.03-4.5-9S9.515 3 12 3m0 0a8.997 8.997 0 0 1 7.843 4.582M12 3a8.997 8.997 0 0 0-7.843 4.582m15.686 0A11.953 11.953 0 0 1 12 10.5c-2.998 0-5.74-1.1-7.843-2.918m15.686 0A8.959 8.959 0 0 1 21 12c0 .778-.099 1.533-.284 2.253m0 0A17.919 17.919 0 0 1 12 16.5c-3.162 0-6.133-.815-8.716-2.247m0 0A9.015 9.015 0 0 1 3 12c0-1.605.42-3.113 1.157-4.418"></path></svg>Rescrape</button></li>"##
            )],
        )?;
        Ok(())
    }
    ...
    async fn test_get_recipe_bold_enabled_ok() -> Result<()> {
        ...
        let recipe_id = Recipe::create(&state.mm, users[0].id, &recipe, &settings).await?;
...
mod tests_delete {
    ...
    async fn test_delete_recipe_does_not_exist() -> Result<()> {
        ...
        assert_sse_message(&mut ws_server, r#"{"notification":{"type":"toast","message":"Recipe could not be deleted.","status":"alert-error","title":"Operation Failed"}}"#).await;
        Ok(())
    }
    ...
    async fn test_delete_recipe_exists_ok() -> Result<()> {
        ...
        let res = server.delete(&base_uri(recipe_id)).await;
...
fn assert_complete_recipe(
    ...
    assert_html(
        ...
        &[
            ...
            &format!(
                r##"<button id="edit-recipe" class="ml-2 hidden sm:block" title="Edit recipe" hx-get="/recipes/{recipe_id}/edit" hx-push-url="true" hx-target="#content" hx-swap="innerHTML transition:true">"##
            ),
            &format!(
            ...
            &format!(
                r##"<a title="Share recipe" hx-post="/recipes/{recipe_id}/share" hx-target="#share-dialog-result" hx-push-url="false""##
            ),
            r#"<li title="Print recipe" _="on click print()">"#,
            &format!(
                r##"<a title="Delete recipe" hx-delete="/recipes/{recipe_id}" hx-swap="none" hx-confirm="Are you sure you wish to delete this recipe?" hx-indicator="#fullscreen-loader">"##
            ),
