...
use test_harness::{assert_html, assert_sse_message, build_server_sse};
...
async fn test_post_add_new_timeline_recipe_not_exist_ok() -> Result<()> {
    ...
    assert_sse_message(&mut ws_server, r#"{"notification":{"type":"toast","message":"Could not create timeline event.","status":"alert-error","title":"Operation Failed"}}"# ).await;
    Ok(())
}
...
async fn test_post_add_new_timeline_ok() -> Result<()> {
    ...
    assert_sse_message(&mut ws_server, r#"{"notification":{"type":"toast","message":"Timeline event created.","status":"alert-info","title":"Success"}}"# ).await;
    Ok(())
}
...
async fn test_get_timeline_event_not_exist_ok() -> Result<()> {
    ...
    res.assert_status_not_found();
    assert_sse_message(&mut ws_server, r#"{"notification":{"type":"toast","message":"Timeline event does not exist.","status":"alert-warning","title":"Attention"}}"# ).await;
    Ok(())
}
