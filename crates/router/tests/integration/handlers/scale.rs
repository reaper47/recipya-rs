...
use test_harness::{TestSse, assert_must_be_logged_in, build_server_sse};
use test_harness::{assert_html, assert_sse_message};
...
async fn setup() -> Result<(TestServer, TestSse, AppState)> {
    let (server, ws_server, state) = build_server_sse(default_config()).await?;
...
async fn test_yield_query_param_must_be_specified() -> Result<()> {
    let (server, _sse_server, _) = setup().await?;
...
async fn test_valid_double_yield() -> Result<()> {
    let (server, _sse_server, state) = setup().await?;
    let user_id = User::all(&state.mm).await?[0].id;
    let settings = UserSettingDetails::get(&state.mm, user_id).await?;
    ...
    assert_html(
        &res,
        &[
            r#"<span class="px-2 [input:checked~&amp;]:opacity-50">2 cup blue spinach</span>"#,
            r#"<span class="px-2 [input:checked~&amp;]:opacity-50">1 tbsp cinnamon</span>"#,
            r#"<span class="px-2 [input:checked~&amp;]:opacity-50">4 lb chicken</span>"#,
            r#"<span class="px-2 [input:checked~&amp;]:opacity-50">1 cup bread loaf</span>"#,
            r#"<span class="px-2 [input:checked~&amp;]:opacity-50">1 tbsp beef broth</span>"#,
            r#"<span class="px-2 [input:checked~&amp;]:opacity-50">15 cup flour</span>"#,
            r#"<span class="px-2 [input:checked~&amp;]:opacity-50">4 big apples</span>"#,
            r#"<span class="px-2 [input:checked~&amp;]:opacity-50">Lots of big apples</span>"#,
            r#"<span class="px-2 [input:checked~&amp;]:opacity-50">5 slices of bacon</span>"#,
            r#"<span class="px-2 [input:checked~&amp;]:opacity-50">4 2/3 cans of bamboo sticks</span>"#,
            r#"<span class="px-2 [input:checked~&amp;]:opacity-50">3can of tomato paste</span>"#,
            r#"<span class="px-2 [input:checked~&amp;]:opacity-50">13 1/2 peanut butter jars</span>"#,
            r#"<span class="px-2 [input:checked~&amp;]:opacity-50">1 1/2 cl of whiskey</span>"#,
            r#"<span class="px-2 [input:checked~&amp;]:opacity-50">1 1/3 tbsp lemon juice</span>"#,
            r#"<span class="px-2 [input:checked~&amp;]:opacity-50">Ground ginger</span>"#,
            r#"<span class="px-2 [input:checked~&amp;]:opacity-50">1 fresh pineapple, cored and cut into 1 1/2-inch pieces</span>"#,
            r#"<span class="px-2 [input:checked~&amp;]:opacity-50">Un sac de chips de 2 kg</span>"#,
            r#"<span class="px-2 [input:checked~&amp;]:opacity-50">Two 30-ounce can Goya beans</span>"#,
            r#"<span class="px-2 [input:checked~&amp;]:opacity-50">8 lb top quality chicken filet</span>"#,
            r#"<span class="px-2 [input:checked~&amp;]:opacity-50">1 1/3 tbsp lemon juice</span>"#,
        ],
    );
    Ok(())
