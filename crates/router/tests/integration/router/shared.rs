...
use models::{
    ...
    share::ShareRecipe,
    shopping::{ShareShoppingList, ShoppingList, ShoppingListItemForCreate},
    user::User,
};
use test_db::default_config;
use test_fixtures::{RecipeImages, insert_other_user};
use test_harness::{
    assert_html, assert_not_in_html, build_server_anonymous, build_server_logged_in,
};
...
mod tests_recipe {
    ...
    async fn test_shared_logged_in_other_user_ok() -> Result<()> {
        let (server, state) = build_server_logged_in(default_config()).await?;
        let user = insert_other_user(&state.mm, "slava@ukraini.ua").await?;
