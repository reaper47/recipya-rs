...
use test_fixtures::{TEST_DEMO_EMAIL, insert_other_user};
use test_harness::{
    assert_html, assert_must_be_logged_in, assert_not_in_html, assert_sse_message,
    build_server_logged_in, build_server_sse, build_server_sse_other_user,
};
...
mod tests_settings {
    ...
    async fn test_server_and_conn_tabs_not_displayed_when_not_admin_ok() -> Result<()> {
        let server = build_server_sse_other_user(default_config(), TEST_DEMO_EMAIL)
            .await?
            .0;
...
mod tests_export {
    ...
    mod tests_post {
        ...
        async fn test_empty_recipe_ids_ok() -> Result<()> {
            ...
            assert_sse_message(
                    &mut ws_server,
                    r#"{"notification":{"type":"toast","message":"No recipes selected for export.","status":"alert-warning","title":"Attention"}}"#,
                )
                .await;
            Ok(())
        }
...
mod tests_themes {
    ...
    async fn test_set_default_theme_ok() -> Result<()> {
        let (server, state) = build_server_logged_in(default_config()).await?;
        let other_user = insert_other_user(&state.mm, "slava@ukraini.ua").await?;
        let users = User::all(&state.mm).await?;
