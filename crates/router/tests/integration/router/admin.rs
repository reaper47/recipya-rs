...
use test_fixtures::{TEST_USER_EMAIL, insert_other_user};
use test_harness::{
    assert_html, assert_must_be_logged_in, assert_sse_message, build_server_logged_in,
    build_server_sse, build_server_sse_other_user,
};

type Result<T> = core::result::Result<T, Box<dyn std::error::Error>>;

mod tests_user {
    use router::schemas::auth::RegisterForm;
    ...
    async fn test_must_be_admin_ok() -> Result<()> {
        let (server, _, _) = build_server_sse_other_user(default_config(), "demo@demo.com").await?;
...
mod tests_users {
    ...
    mod tests_delete {
        ...
        #[tokio::test]
        async fn test_must_be_admin_ok() -> Result<()> {
            let server = build_server_sse_other_user(default_config(), "demo@demo.com")
                .await?
                .0;
        ...
        async fn test_delete_user_ok() -> Result<()> {
            ...
            res.assert_status_ok();
            assert_sse_message(&mut ws_server, r#"{"notification":{"type":"toast","message":"User deleted.","status":"alert-info","title":"Success"}}"# ).await;
    ...
    mod tests_patch {
        use router::schemas::admin::UpdatePasswordForm;
        ...
        async fn test_update_success_ok() -> Result<()> {
            ...
            res.assert_status_ok();
            assert_sse_message(&mut ws_server, r#"{"notification":{"type":"toast","message":"User password updated.","status":"alert-info","title":"Success"}}"# ).await;
            assert_html(
                &res,
                &[&format!(
...
mod tests_user_row {
    ...
    async fn test_must_be_admin_ok() -> Result<()> {
        let (server, _, state) =
            build_server_sse_other_user(default_config(), "demo@demo.com").await?;
        let user_id = User::all(&state.mm).await?[0].id;
