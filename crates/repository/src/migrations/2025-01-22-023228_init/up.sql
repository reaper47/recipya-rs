...
CREATE TABLE user_settings (
  id bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  user_id bigint NOT NULL REFERENCES users (id) ON DELETE CASCADE,
  measurement_system_id smallint NOT NULL REFERENCES measurement_systems (id)
...
(3) NOT NULL,
  ...
  fts_combined tsvector NOT NULL GENERATED ALWAYS AS
    (setweight(to_tsvector(get_tsv_config (
  LANGUAGE), coalesce(name, '')), 'A') ||
    setweight(to_tsvector(get_tsv_config (
  LANGUAGE), coalesce(source, '')), 'B') ||
    setweight(to_tsvector(get_tsv_config (
...
--- Nutrition Databases
---
CREATE TABLE nutrition_databases (
  id bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  name text NOT NULL,
...
BEGIN
  ...
  FROM
    categories
  WHERE
    name IN ('uncategorized', 'appetizers', 'bread',
      'breakfasts', 'condiments', 'dessert', 'lunch',
      'main dish', 'salad', 'side dish', 'snacks',
      'soups', 'stews');
