...
CREATE TABLE nutrition (
  id bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  is_precalculated_by_source boolean NOT NULL DEFAULT FALSE,
...
CREATE TABLE nutrition_per_100g (
  ...
  nutrition_id bigint NOT NULL REFERENCES nutrition (id) ON DELETE CASCADE
);
...
CREATE TABLE user_settings (
  ...
  nutrition_source_id smallint NOT NULL REFERENCES nutrition_sources (id) ON DELETE CASCADE DEFAULT 1,
...
BEGIN
  ...
  SELECT NEW.id,
    id
  FROM categories
  WHERE name IN ('uncategorized', 'appetizers', 'bread', 'breakfasts',
    'condiments', 'dessert', 'lunch', 'main dish', 'salad',
    'side dish', 'snacks', 'soups', 'stews');
...
CREATE OR REPLACE FUNCTION delete_orphaned_nutrition ()
  RETURNS TRIGGER
  AS $$
BEGIN
  ...
  WHERE id = OLD.nutrition_id
    AND NOT EXISTS (
      ...
      WHERE nutrition_id = OLD.nutrition_id)
    AND NOT EXISTS (
      SELECT 1
      FROM nutrition_per_serving
      WHERE nutrition_id = OLD.nutrition_id);
  RETURN OLD;
END;
$$
