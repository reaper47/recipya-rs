...
impl UnitScaler for Unit {
    ...
    fn scale(&self, factor: f64) -> Result<Unit> {
        ...
        match self.unit_type() {
            UnitType::Millimetre => {
                if scaled_value >= 1e6 {
                    Ok(Unit::Kilometre(scaled_value * 1e-6))
                } else if scaled_value >= 1e3 {
                    Ok(Unit::Metre(scaled_value * 1e-3))
                } else if scaled_value >= 10.0 {
                    Ok(Unit::Centimetre(scaled_value * 1e-1))
                } else {
                    Ok(Unit::Millimetre(scaled_value))
                }
            }
            UnitType::Centimetre => {
                if scaled_value >= 1e5 {
                    Ok(Unit::Kilometre(scaled_value * 1e-5))
                } else if scaled_value >= 1e2 {
                    Ok(Unit::Metre(scaled_value * 1e-2))
                } else if scaled_value >= 10.0 {
                    Ok(Unit::Centimetre(scaled_value))
                } else {
                    Ok(Unit::Millimetre(scaled_value * 10.0))
                }
            }
            UnitType::Metre => {
                if scaled_value >= 1e3 {
                    Ok(Unit::Kilometre(scaled_value * 1e-3))
                } else if scaled_value >= 1.0 {
                    Ok(Unit::Metre(scaled_value * 1.0))
                } else if scaled_value >= 1e-2 {
                    Ok(Unit::Centimetre(scaled_value * 1e2))
                } else {
                    Ok(Unit::Millimetre(scaled_value * 1e3))
                }
            }
            UnitType::Kilometre => {
                if scaled_value >= 1.0 {
                    Ok(Unit::Kilometre(scaled_value))
                } else if scaled_value >= 1e-3 {
                    Ok(Unit::Metre(scaled_value * 1e3))
                } else if scaled_value >= 1e-5 {
                    Ok(Unit::Centimetre(scaled_value * 1e5))
                } else {
                    Ok(Unit::Millimetre(scaled_value * 1e6))
                }
            }
            UnitType::Inch => {
                if scaled_value >= 12.0 {
                    Ok(Unit::Foot(scaled_value / 12.0))
                } else  {
                    Ok(Unit::Inch(scaled_value))
                }
            }
            UnitType::Foot => {
                if scaled_value >= 1.0 {
                    Ok(Unit::Foot(scaled_value))
                } else  {
                    Ok(Unit::Inch(scaled_value / 0.083_333_333_333_333_33))
                }
            }
            UnitType::Milligram => {
                if scaled_value >= 1e6 {
...
mod tests {
    ...
    mod test_scale_length {
        use super::*;

        // Millimetre
        #[test]
        fn test_mm_to_km() -> Result<()> {
            assert_eq!(Unit::Millimetre(100.0).scale(1e4)?, Unit::Kilometre(1.0));
            Ok(())
        }

        #[test]
        fn test_mm_to_m() -> Result<()> {
            assert_eq!(Unit::Millimetre(15.0).scale(70.0)?, Unit::Metre(1.05));
            Ok(())
        }

        #[test]
        fn test_mm_to_cm() -> Result<()> {
            assert_eq!(Unit::Millimetre(15.0).scale(10.0)?, Unit::Centimetre(15.0));
            Ok(())
        }

        #[test]
        fn test_mm_to_mm() -> Result<()> {
            assert_eq!(Unit::Millimetre(3.0).scale(2.0)?, Unit::Millimetre(6.0));
            Ok(())
        }
        ...
        fn test_cm_to_km() -> Result<()> {
            assert_eq!(Unit::Centimetre(100.0).scale(1e4)?, Unit::Kilometre(10.0));
            Ok(())
        }

        #[test]
        fn test_cm_to_m() -> Result<()> {
            assert_eq!(Unit::Centimetre(15.0).scale(60.0)?, Unit::Metre(9.00));
            Ok(())
        }

        #[test]
        fn test_cm_to_cm() -> Result<()> {
            assert_eq!(Unit::Centimetre(15.0).scale(2.0)?, Unit::Centimetre(30.0));
            Ok(())
        }

        #[test]
        fn test_cm_to_mm() -> Result<()> {
            assert_approx_eq(Unit::Centimetre(3.0).scale(0.2)?, Unit::Millimetre(6.0), 1e-10);
            Ok(())
        }
        ...
        fn test_m_to_km() -> Result<()> {
            assert_eq!(Unit::Metre(100.0).scale(10.0)?, Unit::Kilometre(1.0));
            Ok(())
        }

        #[test]
        fn test_m_to_m() -> Result<()> {
            assert_eq!(Unit::Metre(15.0).scale(60.0)?, Unit::Metre(900.0));
            Ok(())
        }

        #[test]
        fn test_m_to_cm() -> Result<()> {
            assert_eq!(Unit::Metre(1.0).scale(0.5)?, Unit::Centimetre(50.0));
            Ok(())
        }

        #[test]
        fn test_m_to_mm() -> Result<()> {
            assert_approx_eq(Unit::Metre(1.0).scale(0.002)?, Unit::Millimetre(2.0), 1e-10);
            Ok(())
        }
        ...
        fn test_km_to_km() -> Result<()> {
            assert_eq!(Unit::Kilometre(1.0).scale(10.0)?, Unit::Kilometre(10.0));
            Ok(())
        }

        #[test]
        fn test_km_to_m() -> Result<()> {
            assert_eq!(Unit::Kilometre(1.0).scale(0.5)?, Unit::Metre(500.0));
            Ok(())
        }

        #[test]
        fn test_km_to_cm() -> Result<()> {
            assert_eq!(Unit::Kilometre(1.0).scale(1e-5)?, Unit::Centimetre(1.0));
            Ok(())
        }

        #[test]
        fn test_km_to_mm() -> Result<()> {
            assert_approx_eq(Unit::Kilometre(1.0).scale(0.000005)?, Unit::Millimetre(5.0), 1e-10);
            Ok(())
        }
        ...
        fn test_in_to_in() -> Result<()> {
            assert_eq!(Unit::Inch(2.0).scale(4.0)?, Unit::Inch(8.0));
            Ok(())
        }

        #[test]
        fn test_in_to_ft() -> Result<()> {
            assert_eq!(Unit::Inch(3.0).scale(4.0)?, Unit::Foot(1.0));
            Ok(())
        }
        ...
        fn test_ft_to_in() -> Result<()> {
            assert_eq!(Unit::Foot(1.0).scale(0.5)?, Unit::Inch(6.0));
            Ok(())
        }

        #[test]
        fn test_ft_to_ft() -> Result<()> {
            assert_eq!(Unit::Foot(1.0).scale(5.0)?, Unit::Foot(5.0));
            Ok(())
        }
    }
