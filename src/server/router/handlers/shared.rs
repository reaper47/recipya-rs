...
pub async fn share_recipe_handler(ctx: CtxW, header_map: HeaderMap, OriginalUri(uri): OriginalUri, State(state): State<AppState>, Path(link): Path<Uuid>) -> impl IntoResponse {
    ...
    templates::recipes::view_recipe(uri.path(), state.data_dir, Data {
        is_admin: user_id == 1,
        is_authenticated: user_id > 0,
        is_autologin: state.config.is_autologin,
        is_hx_request: is_hx_request(&header_map),
        about: AboutData { is_update_available: false },
        pagination: None,
        searchbar: None,
