    ...
    mod tests_duplicate_recipes {
        ...
        fn assert_recipe_form(res: TestResponse) {
            let expected = [
                ...
                r#"<textarea required name="instruction" rows="4" class="textarea textarea-bordered w-full" placeholder="Mix all ingredients together" _="on keydown if event.key is 'Enter' halt the event then call addItem(event)">Turn the oven at 300 F</textarea>"#,
                r#"<textarea required name="instruction" rows="4" class="textarea textarea-bordered w-full" placeholder="Mix all ingredients together" _="on keydown if event.key is 'Enter' halt the event then call addItem(event)">Soak the chicken in the lemon juice</textarea>"#,
                r#"<textarea required name="instruction" rows="4" class="textarea textarea-bordered w-full" placeholder="Mix all ingredients together" _="on keydown if event.key is 'Enter' halt the event then call addItem(event)">Mix all these ingredients</textarea>"#,
