...
use crate::core::support::b64::{b64u_decode_to_string, b64u_encode};
use crate::core::support::time::{now_utc_plus_sec_str, parse_utc};

pub use self::error::{Error, Result};

/// Represents a web token consisting of an identifier, expiration date, and signature.
///
/// A `Token` is typically used for authentication or authorization purposes, and it follows a
/// JWT-like format, consisting of three parts: a base64url-encoded identifier, an expiration date
/// (also base64url-encoded in RFC3339 format), and a signature (base64url-encoded).
#[derive(Debug)]
...
fn _validate_token_sign_and_exp(origin_token: &Token, salt: Uuid, key: &[u8]) -> Result<()> {
    ...
    if origin_exp < OffsetDateTime::now_utc() {
