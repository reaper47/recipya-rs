...
use crate::core::scraper::schema::{
    ...
    NumberOrText, RecipeCategory, RecipeSchema,
};
...
impl From<&Recipe> for RecipeSchema {
    fn from(r: &Recipe) -> Self {
        ...
        .chain((!r.notes.trim().is_empty()).then(|| {
            (
                "Notes".to_string(),
                r.notes.split("\n\n").map(str::to_string).collect(),
            )
        ...
        let cook = humantime::parse_duration(&r.cook_time).unwrap_or_default();
        let prep = humantime::parse_duration(&r.prep_time).unwrap_or_default();

        let ingredients = r.ingredients.lines().collect::<Vec<_>>();
        let url = if let Ok(u) = Url::parse(&r.source_url) {
        ...
        Self {
            ...
            date_created: Some(DateOrDateTime::Date(
                iso8601::Date::from_str(r.created.split_whitespace().next().unwrap_or_default())
                    .unwrap_or_default(),
            )),
            description: to_text(r.description.clone()),
            ...
            is_based_on: to_is_based_on(if !r.source.is_empty() {
                format!("{} [Imported from Paprika]", r.source)
            } else {
            ...
            recipe_yield: to_yield(r.servings.parse().unwrap_or(1)),
